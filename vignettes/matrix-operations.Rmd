...
Apply any function element-wise, or row/column-wise based on activation:

```{r custom-transform}
# Element-wise: square root
expr_sqrt <- expr_tm |>
  activate(matrix) |>
  transform_matrix(sqrt)

cat("After sqrt transformation:\n")
cat("  Range:", range(expr_sqrt$matrix), "\n")

# Row-wise: rank values within each gene
expr_ranked <- expr_tm |>
  activate(rows) |>
  transform_matrix(rank)

cat("\nAfter row-wise ranking:\n")
cat("  Range:", range(expr_ranked$matrix), "\n")
```
...
| **Custom (element-wise)** | `activate(matrix) \|> transform_matrix()` | Apply any function to matrix |
| **Custom (row-wise)** | `activate(rows) \|> transform_matrix()` | Apply function per row |
| **Custom (column-wise)** | `activate(columns) \|> transform_matrix()` | Apply function per column |
